...
export const ServerRoute = createServerFileRoute("/api/roles/").methods({
  ...
  try {
    ...
    const whereClause = search ? like(roles.name, `%${search}%`) : undefined;
    ...
    const orderBy =
      sortOrder === "asc"
        ? asc(roles[sortBy as keyof typeof roles])
        : desc(roles[sortBy as keyof typeof roles]);
  ...
  try {
    ...
    // Check if user has permission to manage roles (simplified for auth schema)
    const currentUser = await db
      .select({
        role: user.role,
      })
