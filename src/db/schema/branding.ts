...
export const branding = pgTable(
  'branding',
  {
    ...
    logoDisplayMode: text('logo_display_mode', { enum: ['logo-only', 'logo-with-name', 'name-only'] }).default('logo-with-name'),
...
export const insertBrandingSchema = createInsertSchema(branding, {
  ...
  logoDisplayMode: v.optional(v.picklist(['logo-only', 'logo-with-name', 'name-only'])),
  primaryColor: v.optional(v.pipe(v.string(), v.regex(/^#(?:[0-9A-Fa-f]{3}){1,2}(?:[0-9A-Fa-f]{2})?$/, 'Must be a valid hex color'))),
  secondaryColor: v.optional(v.pipe(v.string(), v.regex(/^#(?:[0-9A-Fa-f]{3}){1,2}(?:[0-9A-Fa-f]{2})?$/, 'Must be a valid hex color'))),
  accentColor: v.optional(v.pipe(v.string(), v.regex(/^#(?:[0-9A-Fa-f]{3}){1,2}(?:[0-9A-Fa-f]{2})?$/, 'Must be a valid hex color'))),
  backgroundColor: v.optional(v.pipe(v.string(), v.regex(/^#(?:[0-9A-Fa-f]{3}){1,2}(?:[0-9A-Fa-f]{2})?$/, 'Must be a valid hex color'))),
  textColor: v.optional(v.pipe(v.string(), v.regex(/^#(?:[0-9A-Fa-f]{3}){1,2}(?:[0-9A-Fa-f]{2})?$/, 'Must be a valid hex color'))),
